<script>
  ...
  /** @type {number} */
  export let userId;
  /** When true, hide page chrome (back / switch-user) for split-pane use */
  export let embedded = false;

  let currentUser;
  let activeUserId = Number(userId);
  ...
  onMount(async () => {
    ...
    await reloadAll({ soft: false });
  });

  $: if (Number(userId) && Number(userId) !== activeUserId) {
    activeUserId = Number(userId);
    if (currentUser) reloadAll({ soft: true });
  }

  async function reloadAll({ soft = false } = {}) {
    if (!activeUserId) return;
    const gen = ++loadGen;
    const softMode = soft && !!user;

    if (softMode) {
      switching = true;
    } else {
      initialLoad = true;
      user = null; stats = null; detail = null; queries = [];
      userLoading = true; statsLoading = true; detailLoading = true; listLoading = true;
    }
    ...
    try {
      const [nextUser] = await Promise.all([
        authApiFetch(`${API_ROUTES.USER}/${activeUserId}`).catch(e => {
      ...
      const canHelper     = currentUser.role === "master" || (currentUser.queryAccessTechHelper ?? true);
      if (
        (nextUser.subRole === "telecaller" && !canTelecaller) ||
        (nextUser.subRole === "tech" && !canTech) ||
        (nextUser.subRole === "tech_helper" && !canHelper)
      ) {
        if (!embedded) goto("/admin/query/user");
        return;
      }

      const dashParams = buildParamsFor(nextUser, { page: null });
      const listParams = buildParamsFor(nextUser, { page: currentPage, limit: rowsPerPage });
      const detailParams = buildDetailParams();

      const skipList = selectedFilter === "custom" && (!customStartDate || !customEndDate);
      const [nextStats, nextDetail, listRes] = await Promise.all([
        authApiFetch(`${API_ROUTES.QUERY}/dashboard?${dashParams}`).catch(() => null),
        authApiFetch(`${API_ROUTES.QUERY}/user-detail/${activeUserId}?${detailParams}`).catch(() => null),
        skipList ? Promise.resolve(null) : authApiFetch(`${API_ROUTES.QUERY}?${listParams}`).catch(() => null),
      ]);
      if (gen !== loadGen) return;

      // Single atomic paint — prevents layout shake
      user = nextUser;
      if (nextStats) stats = nextStats;
      else if (!softMode) stats = null;
      if (nextDetail) detail = nextDetail;
      else if (!softMode) detail = null;
      if (listRes) {
        queries = listRes.data ?? [];
        totalItems = listRes.total ?? 0;
        totalPages = listRes.totalPages ?? 0;
      } else if (!softMode) {
        queries = []; totalItems = 0; totalPages = 0;
      }
    } finally {
      if (gen === loadGen) {
        switching = false;
  ...
  function buildParamsFor(u, extra = {}) {
    const dp = buildDateParams(), p = new URLSearchParams();
    if (!u) return p;
    if (u.subRole === "telecaller") p.set("raisedById", String(activeUserId));
    else p.set("assignedToId", String(activeUserId));
  ...
  async function loadUser() {
    userLoading = true;
    try { user = await authApiFetch(`${API_ROUTES.USER}/${activeUserId}`); }
  ...
  function buildUserParams(extra = {}) {
    const dp = buildDateParams(), p = new URLSearchParams();
    if (!user) return p;
    if (user.subRole === "telecaller") p.set("raisedById",   String(activeUserId));
    else                               p.set("assignedToId", String(activeUserId));
    if (dp.dateFrom)    p.set("dateFrom",  dp.dateFrom);
    if (dp.dateTo)      p.set("dateTo",    dp.dateTo);
    if (filterStatus)   p.set("status",    filterStatus);
  ...
  async function loadDetail() {
    ...
    try {
      ...
      detail = await authApiFetch(`${API_ROUTES.QUERY}/user-detail/${activeUserId}?${q}`);
  ...
  function subRoleLabel(sr) {
    if (sr === "telecaller") return "Telecaller";
    if (sr === "tech_helper") return "Senior Tech";
    if (sr === "tech") return "Tech";
    return sr ?? "-";
  }
...
<div class={embedded ? "qud-embedded qud-page" : "page-wrapper qud-page"}>
  <div class={embedded ? "qud-content" : "content"}>

    {#if switching}
      <div class="qud-switch-bar" aria-hidden="true"><div class="qud-switch-bar-inner"></div></div>
    {/if}

    <!-- Back + title (standalone page only) -->
    {#if !embedded}
      <div class="d-flex align-items-center gap-2 mb-3">
        <button class="btn btn-sm btn-outline-secondary" on:click={() => history.back()}>
          <i class="ti ti-arrow-left"></i> Back
        </button>
        <h4 class="mb-0">Query User Dashboard</h4>
      </div>
    {/if}

    <!-- ── User header / profile ─────────────────────────────────────────────── -->
    {#if initialLoad && !user}
      <div class="user-header-card mb-3">
        <div class="d-flex gap-3 align-items-center">
          <div class="qud-skel-block" style="width:36px;height:36px;border-radius:8px;"></div>
          <div class="flex-grow-1">
            <div class="qud-skel-block mb-2" style="width:40%;height:14px;"></div>
            <div class="qud-skel-block" style="width:24%;height:10px;"></div>
          </div>
          <div class="qud-skel-block" style="width:96px;height:28px;border-radius:6px;"></div>
        </div>
        <div class="d-flex gap-2 mt-3 pt-3 border-top">
          <div class="qud-skel-block" style="width:120px;height:28px;"></div>
          <div class="qud-skel-block" style="width:140px;height:28px;"></div>
          <div class="qud-skel-block" style="width:100px;height:28px;"></div>
        </div>
      </div>
    {:else if user}
      <div class="user-header-card mb-3 pdf-section">
        <div class="d-flex align-items-center gap-3">
          <div class="user-avatar user-avatar--{user.subRole ?? 'other'} flex-shrink-0">{maskedUserInitials}</div>
          <div class="flex-grow-1 min-w-0">
            <div class="d-flex align-items-center gap-2 flex-wrap">
              <span class="qud-uname">{maskedUserName}</span>
          ...
          <div class="qud-profile-actions flex-shrink-0">
            {#if !embedded}
              <select class="form-select form-select-sm qud-switch-select" class:pdf-hide={exportingPdf}
            ...
            <button
              type="button"
              class="btn btn-sm btn-outline-success qud-export-btn"
              data-html2canvas-ignore="true"
              on:click={exportPdf}
              disabled={exportingPdf || switching || !stats || !detail}
              title="Export PDF"
            >
              {#if exportingPdf}
                <span class="spinner-border spinner-border-sm me-1" style="width:11px;height:11px;"></span>Generating…
              {:else}
        ...
        <div class="d-flex align-items-center gap-2 flex-wrap mt-3 pt-3 border-top" class:pdf-hide={exportingPdf}>
          ...
          <button class="btn btn-sm btn-outline-primary ms-auto"
            on:click={() => reloadAll({ soft: true })} disabled={switching || statsLoading || detailLoading}>
    ...
    <!-- ── Stats cards ────────────────────────────────────────────────────────── -->
    {#if initialLoad && statsLoading}
      <div class="row g-2 mb-3">
        {#each Array(6) as _}
          <div class="col-6 col-md-2"><div class="qud-skel-block" style="height:72px;border-radius:8px;"></div></div>
        {/each}
      </div>
    {:else if stats && user}
    ...
    <!-- ── Detailed analytics ─────────────────────────────────────────────────── -->
    {#if initialLoad && detailLoading}
      <div class="row g-2 mb-3">
        <div class="col-lg-6"><div class="qud-skel-block" style="height:160px;border-radius:8px;"></div></div>
        <div class="col-lg-3"><div class="qud-skel-block" style="height:160px;border-radius:8px;"></div></div>
        <div class="col-lg-3"><div class="qud-skel-block" style="height:160px;border-radius:8px;"></div></div>
      </div>
    {:else if detail}

      <!-- Row 1: Trend chart + Resolution rate + Type breakdown -->
      <div class="row g-2 mb-3 pdf-section">
    ...
    <div class="d-flex align-items-center justify-content-between mb-2">
      ...
      <span class="text-muted">{totalItems} total</span>
    </div>
    {#if initialLoad && listLoading}
      <div class="qud-skel-block mb-2" style="height:180px;border-radius:8px;"></div>
    {:else if listLoading && !queries.length}
      <div class="text-center py-4 text-muted">Loading queries…</div>
    {:else if queries.length === 0}
      <div class="text-center py-4 text-muted">No queries found.</div>
    {:else}
      <div class="card border-0 shadow-sm mb-0">
        <div class="card-body p-2">
          <div class="table-responsive">
            <table class="table table-hover align-middle mb-0 qud-table">
              <thead class="table-light">
                <tr>
                  <th>#</th><th>Subject</th>
                  <th>{isTelecaller(user) ? "Assigned To" : "Raised By"}</th>
                  <th>Type</th><th>Priority</th><th>Status</th>
                  <th>Final Quot.</th><th>Delay</th><th>Raised At</th><th></th>
                </tr>
              </thead>
              <tbody>
                {#each queries as q, i}
                  <tr>
                    ...
                    <td style="max-width:220px;">
                      {#if q.ticketCode}
                        <div class="qud-ticket">{q.ticketCode}</div>
                      {/if}
                    ...
                    {/if}
                    <td><span class="badge bg-light text-dark border">{typeLabel(q.type)}</span></td>
                    <td><span class={PRIORITY_COLORS[q.priority] ?? "badge bg-secondary"}>{q.priority ?? "-"}</span></td>
                    <td><span class={STATUS_COLORS[q.status] ?? "badge bg-secondary"}>{q.status?.replace("_"," ")}</span></td>
                    <td>
                      {#if q.hasFinalQuotation}
                        <span class="badge bg-success"><i class="ti ti-flag-check me-1"></i>Sent</span>
                      {:else}
                    ...
                    <td>
                      {#if q.slaBreached}
                        <span class="badge bg-danger"><i class="ti ti-clock-exclamation me-1"></i>{fmtMins(q.firstResponseMins)}</span>
                      {:else if q.firstResponseMins !== null && q.firstResponseMins !== undefined}
                        <span class="badge bg-success">{fmtMins(q.firstResponseMins)}</span>
                      {:else}
...
<style>
  ...
  .qud-page,
  .qud-page :global(.content),
  .qud-page :global(.qud-content) {
  ...
  .qud-page :global(.fw-bold),
  .qud-page :global(.fw-semibold) {
    font-weight: 500 !important;
  }
  .qud-page :global(.form-control),
  .qud-page :global(.form-select),
  .qud-page :global(.btn),
  .qud-page :global(.btn-sm) {
  ...
  .qud-page :global(.text-muted),
  .qud-page :global(.small) {
    font-size: 11px !important;
    font-weight: 400 !important;
  }
  .qud-page :global(.badge) {
  ...
  .qud-uname { font-size: 14px; font-weight: 500; color: #212529; }
  .qud-email { font-size: 11px !important; }

  .qud-content { position: relative; }
  .qud-switch-bar {
    position: absolute; top: 0; left: 0; right: 0; z-index: 6;
    height: 2px; overflow: hidden; pointer-events: none;
  }
  .qud-switch-bar-inner {
    height: 100%; width: 35%;
    background: linear-gradient(90deg, transparent, #3b5bdb, transparent);
    animation: qud-slide 0.85s linear infinite;
  }
  @keyframes qud-slide {
  ...
  .qud-skel-block {
    ...
    animation: qud-shimmer 1.2s ease-in-out infinite;
  }
  @keyframes qud-shimmer {
  ...
  .user-avatar {
    ...
    font-weight: 600; font-size: 12px; flex-shrink: 0; color: #fff;
  }
  .user-avatar--telecaller { background: #f59f00; }
  .user-avatar--tech { background: #0ca678; }
  .user-avatar--tech_helper { background: #7950f2; }
  .user-avatar--other { background: #6c757d; }

  .stat-card {
    background: #fff; border-radius: 8px; border: 1px solid #e9ecef;
    box-shadow: none; padding: 10px 8px; text-align: center;
    border-top: 2px solid transparent;
  }
  .stat-icon { font-size: 14px; opacity: 0.45; margin-bottom: 2px; }
  .stat-value { font-size: 16px; font-weight: 600; line-height: 1.1; }
  .stat-label { font-size: 10px; color: #868e96; margin-top: 2px; font-weight: 400; }
  ...
  .perf-card {
    background: #fff; border-radius: 8px; border: 1px solid #e9ecef;
    box-shadow: none; padding: 12px; border-left: 3px solid transparent; height: 100%;
  }
  ...
  .perf-card-title {
    ...
    margin-bottom: 8px; padding-bottom: 6px; border-bottom: 1px solid #f1f3f5;
  }
  .perf-row {
    display: flex; align-items: center; justify-content: space-between; gap: 8px;
    padding: 3px 0; border-bottom: 1px solid #f8f9fa; font-size: 11.5px;
  }
  .perf-row:last-child { border-bottom: none; }
  .perf-label { color: #868e96; font-weight: 400; }
  .perf-val { font-weight: 500; color: #343a40; }
  .perf-badge { margin-left: auto; font-size: 10px; font-weight: 500; padding: 1px 6px; border-radius: 4px; }
  ...
  .detail-card-title {
    font-size: 11px; font-weight: 600; color: #495057;
    display: flex; align-items: center; gap: 5px;
    margin-bottom: 10px; padding-bottom: 8px; border-bottom: 1px solid #f1f3f5;
  }
  .detail-card-title i { font-size: 13px; opacity: 0.7; }
  .empty-hint { text-align: center; color: #adb5bd; font-size: 12px; padding: 16px 0; font-weight: 400; }

  .trend-chart { display: flex; align-items: flex-end; gap: 4px; height: 110px; overflow-x: auto; padding-bottom: 4px; }
  .trend-col { display: flex; flex-direction: column; align-items: center; gap: 2px; min-width: 26px; flex: 1; }
  .trend-bars { display: flex; align-items: flex-end; gap: 2px; height: 90px; width: 100%; justify-content: center; }
  .trend-bar { width: 9px; border-radius: 2px 2px 0 0; transition: height 0.3s; min-height: 2px; }
  .trend-bar--total    { background: #c5cff9; }
  .trend-bar--resolved { background: #3b5bdb; }
  .trend-label { font-size: 9px; color: #adb5bd; white-space: nowrap; font-weight: 400; }
  .trend-legend { display: flex; align-items: center; gap: 4px; margin-top: 6px; font-size: 10.5px; color: #6c757d; font-weight: 400; }
  .trend-legend-dot { display: inline-block; width: 8px; height: 8px; border-radius: 2px; }
  .trend-legend-dot--total    { background: #c5cff9; }
  .trend-legend-dot--resolved { background: #3b5bdb; }

  .res-rate-wrap { position: relative; width: 88px; height: 88px; margin: 0 auto 10px; }
  ...
  .res-rate-pct { font-size: 15px; font-weight: 600; color: #198754; line-height: 1; }
  .res-rate-sub { font-size: 10px; color: #868e96; font-weight: 400; }
  .res-stats { display: flex; flex-direction: column; gap: 2px; }
  .res-stat-row { display: flex; justify-content: space-between; align-items: center; font-size: 11.5px; padding: 3px 0; border-bottom: 1px solid #f8f9fa; }
  .res-stat-row:last-child { border-bottom: none; }
  .vs-team-badge {
    margin-top: 6px; font-size: 10.5px; font-weight: 500; padding: 3px 8px; border-radius: 4px;
  ...
  .type-row { display: flex; align-items: center; gap: 8px; font-size: 11.5px; }
  .type-name { width: 100px; flex-shrink: 0; color: #495057; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 400; }
  .type-bar-wrap { flex: 1; background: #f1f3f5; border-radius: 3px; height: 6px; overflow: hidden; }
  .type-bar { height: 100%; background: #3b5bdb; border-radius: 3px; transition: width 0.3s; }
  .type-count { width: 22px; text-align: right; font-weight: 500; color: #343a40; flex-shrink: 0; }

  .dist-bar-wrap { display: flex; height: 12px; border-radius: 4px; overflow: hidden; gap: 1px; }
  .dist-seg { border-radius: 2px; transition: flex 0.3s; }
  ...
  .dist-legend { display: flex; align-items: center; gap: 3px; font-size: 10.5px; color: #6c757d; flex-wrap: wrap; font-weight: 400; }
  ...
  .dist-stat { background: #f8f9fa; border-radius: 6px; padding: 6px 10px; display: flex; flex-direction: column; gap: 1px; }
  .dist-stat-label { font-size: 10px; color: #adb5bd; font-weight: 400; }
  .dist-stat-val { font-size: 13px; font-weight: 500; color: #343a40; }

  .heatmap-grid { display: grid; grid-template-columns: repeat(12, 1fr); gap: 3px; }
  .heatmap-cell { border-radius: 4px; padding: 3px 2px; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: default; min-height: 32px; }
  .heatmap-hr { font-size: 9px; color: #6c757d; font-weight: 400; }
  .heatmap-count { font-size: 9px; font-weight: 500; color: #3b5bdb; margin-top: 1px; }
  .heatmap-legend { display: flex; align-items: center; gap: 3px; }
  .heatmap-legend-cell { width: 12px; height: 8px; border-radius: 2px; }

  .btn-xs { padding: 2px 6px; font-size: 10.5px !important; }

  .mistake-list { display: flex; flex-direction: column; gap: 5px; }
  .mistake-row { display: flex; align-items: flex-start; gap: 8px; padding: 7px 8px; border-radius: 6px; background: #fafafa; border: 1px solid #f1f3f5; }
  .mistake-row--missed { border-left: 3px solid #dc3545; }
  .mistake-row--slow   { border-left: 3px solid #fd7e14; }
  .mistake-icon { font-size: 14px; flex-shrink: 0; margin-top: 1px; }
  .mistake-row--missed .mistake-icon { color: #dc3545; }
  .mistake-row--slow   .mistake-icon { color: #fd7e14; }
  .mistake-body { flex: 1; display: flex; flex-direction: column; gap: 1px; min-width: 0; }
  .mistake-subject { font-size: 12px; font-weight: 500; color: #212529; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .mistake-meta { font-size: 10.5px; color: #868e96; font-weight: 400; }

  .qud-table { font-size: 11.5px !important; }
  .qud-table :global(th) {
    font-size: 10px !important; font-weight: 600 !important; color: #868e96;
    text-transform: uppercase; letter-spacing: 0.3px; white-space: nowrap;
  }
  .qud-table :global(td) { font-weight: 400; vertical-align: middle; }
  .qud-ticket { font-size: 10px; font-weight: 500; color: #868e96; letter-spacing: 0.2px; margin-bottom: 1px; }
  .qud-qsubj { font-weight: 500 !important; font-size: 12px; }

  .qud-embedded { height: 100%; min-height: 0; position: relative; }
  .qud-content {
    height: 100%; overflow-y: auto; padding: 0 2px 16px 0;
    position: relative;
  }
</style>

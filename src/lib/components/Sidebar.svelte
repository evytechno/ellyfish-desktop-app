  ...
  onMount(() => {
    ...
    const mouseOverHandler = function (e) {
      if ($("body").hasClass("mini-sidebar") && $("#toggle_btn").is(":visible")) {
        const insideSidebar = $(e.target).closest(".sidebar, .header-left").length;
        if (insideSidebar) {
    ...
    $(document).on("click", ".sidebar-close, .sidebar-overlay", closeMenuHandler);
    ...
    return () => {
      ...
      $(document).off("click", ".sidebar-close, .sidebar-overlay", closeMenuHandler);
...
<div class="sidebar no-print" id="sidebar">
  ...
  <div class="sidebar-logo flex items-center h-[57px]">
    <div>
      <a href="/admin/dashboard" class="logo logo-normal">
        {#if setting?.logo}
          <img src={ATTACHMENT_BASE_URL + setting?.logo} class="h-[50px]" alt={setting?.title} />
        {/if}
      </a>
      <a href="/admin/dashboard" class="logo-small">
        {#if setting?.logoSmall}
          <img src={ATTACHMENT_BASE_URL + setting?.logoSmall} alt={setting?.title} />
        {/if}
      </a>
  ...
  <div class="sidebar-inner simplebar-scrollable-y pb-14" data-simplebar="init">
              ...
              <div id="sidebar-menu" class="sidebar-menu">
                <ul>
                  ...
                  <li>
                    <ul>
                      {#if currentUser?.role != "user"}
                        <li class:active={currentPath.startsWith("/admin/admin-dashboard")}>
                          <a href="/admin/admin-dashboard" class:active={currentPath.startsWith("/admin/admin-dashboard")}>
                            <i class="ti ti-dashboard"></i><span>Admin Dashboard</span>
                          </a>
                        </li>
                      {/if}
                      <li class:active={currentPath.startsWith("/admin/dashboard")}>
                        <a href="/admin/dashboard" class:active={currentPath.startsWith("/admin/dashboard")}>
                  ...
                  <li>
                    <ul>
                      {#if currentUser?.role != "user"}
                        <li class:active={currentPath.startsWith("/admin/category")}>
                          <a href="/admin/category" class:active={currentPath.startsWith("/admin/category")}>
                            <i class="ti ti-building"></i><span>Category</span>
                          </a>
                        </li>
                        <li class:active={currentPath.startsWith("/admin/company")}>
                          <a href="/admin/company" class:active={currentPath.startsWith("/admin/company")}>
                            <i class="ti ti-building"></i><span>Companies</span>
                          </a>
                        </li>
                        <li class:active={currentPath.startsWith("/admin/user")}>
                          <a href="/admin/user" class:active={currentPath.startsWith("/admin/user")}>
                      ...
                      {#if currentUser?.subRole === "tech"}
                        <li class="submenu">
                          <a href="#queries" class:active={currentPath.startsWith("/admin/query")}>
                            ...
                            {#if $openQueryCount > 0}
                              <span class="open-queue-badge">{$openQueryCount > 99 ? "99+" : $openQueryCount}</span>
                            {/if}
                            <span class="menu-arrow"></span>
                          </a>
                          <ul>
                            <li>
                              <a href="/admin/query/open" class:active={currentPath === "/admin/query/open"}>
                                Open Queue
                                {#if $openQueryCount > 0}
                                  <span class="open-queue-badge ms-1">{$openQueryCount > 99 ? "99+" : $openQueryCount}</span>
                                {/if}
                              </a>
                            </li>
                            <li>
                              <a href="/admin/query/assigned" class:active={currentPath === "/admin/query/assigned"}>My Assigned</a>
                            </li>
                          </ul>
                        </li>
                      ...
                      <!-- Orders — hidden from tech unless they have orderAccess -->
                      {#if currentUser?.subRole !== "tech" || currentUser?.orderAccess}
                        <li class="submenu">
                          <a href="#orders" class:active={currentPath.startsWith("/admin/order")}>
                            <i class="ti ti-medal"></i><span>Orders</span>
                            <span class="menu-arrow"></span>
                          </a>
                          <ul>
                            <li>
                              <a href="/admin/order" class:active={currentPath === "/admin/order"}>Orders</a>
                            </li>
                            <li>
                              <a href="/admin/order-list/excel" class:active={currentPath === "/admin/order-list/excel"}>Excel Orders</a>
                            </li>
                            <li>
                              <a href="/admin/order/last-activity" class:active={currentPath === "/admin/order/last-activity"}>No Follow Up</a>
                            </li>
                            <li>
                              <a href="/admin/order/won" class:active={currentPath === "/admin/order/won"}>Won</a>
                            </li>
                            <li>
                              <a href="/admin/order-list/dispatched" class:active={currentPath === "/admin/order-list/dispatched"}>Dispatched</a>
                            </li>
                            <li>
                              <a href="/admin/order-list/completed" class:active={currentPath === "/admin/order-list/completed"}>Completed</a>
                            </li>
                            <li>
                              <a href="/admin/order/lost" class:active={currentPath === "/admin/order/lost"}>Lost</a>
                            </li>
                          </ul>
                        </li>
                        <li class:active={currentPath.startsWith("/admin/invoice")}>
                          <a href="/admin/invoice" class:active={currentPath.startsWith("/admin/invoice")}>
                            <i class="ti ti-invoice"></i><span>Invoice (PI)</span>
                          </a>
                        </li>
                        <li class:active={currentPath.startsWith("/admin/workorder")}>
                          <a href="/admin/workorder" class:active={currentPath.startsWith("/admin/workorder")}>
                            <i class="ti ti-invoice"></i><span>Work Order</span>
                          </a>
                        </li>
                        <li class:active={currentPath.startsWith("/admin/payment")}>
                          <a href="/admin/payment" class:active={currentPath.startsWith("/admin/payment")}>
                            <i class="ti ti-wallet"></i><span>Employee Expenses</span>
                          </a>
                        </li>
                        <li class:active={currentPath.startsWith("/admin/stock")}>
                          <a href="/admin/stock" class:active={currentPath.startsWith("/admin/stock")}>
                      ...
                      {#if currentUser?.role != "user"}
                        <li class:active={currentPath.startsWith("/admin/history")}>
                          <a href="/admin/history" class:active={currentPath.startsWith("/admin/history")}>
                            <i class="ti ti-clock"></i><span>User History</span>
                          </a>
                        </li>
                      {/if}
                      {#if currentUser?.role === "master"}
                        <li class:active={currentPath.startsWith("/admin/setting")}>
                          <a href="/admin/setting" class:active={currentPath.startsWith("/admin/setting")}>
    ...
    <div class="simplebar-track simplebar-vertical" style="visibility: visible;">
      <div class="simplebar-scrollbar" style="height: 101px; transform: translate3d(0px, 0px, 0px); display: block;"></div>
    </div>
  </div>

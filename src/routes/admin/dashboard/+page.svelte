<script>
  ...
  async function fetchUpcomingVisits() {
    visitsLoading = true;
    try {
      const data = await authApiFetch(`${API_ROUTES.CLIENT_VISIT}/upcoming?days=30`, {
        method: "GET",
      });
  ...
  async function fetchOrdersStats() {
    try {
      ...
      const data = await authApiFetch(`${API_ROUTES.ORDER}/stats?${query.toString()}`, {
        method: "GET",
      });
      loading = false;
      dashboardData = { ...data };
  ...
  async function fetchOrders() {
    try {
      ...
      const data = await authApiFetch(`${API_ROUTES.ORDER}?${query.toString()}`, {
        method: "GET",
      });
  ...
  async function fetchActivity() {
    try {
      ...
      const data = await authApiFetch(`${API_ROUTES.ORDER_ACTIVITY}?${query.toString()}`, {
        method: "GET",
      });
  ...
  function checkFetchRecord() {
    if (
      firstLoad &&
      ((currentUser?.subRole !== "tech" && currentUser?.subRole !== "tech_helper") ||
        currentUser?.orderAccess)
    ) {
      if (selectedFilter === "custom" && (!customStartDate || !customEndDate)) {
        loadingData = false;
        return;
      ...
      loadingData = true;
      Promise.all([fetchOrders(), fetchActivity(), fetchOrdersStats()]).finally(() => {
        loadingData = false;
      });
    }
  }
  ...
  <div class="content dash-layout pb-0">
    <div class="dash-main">
      ...
      {#if $missedReminderCount > 0}
        <div
          class="alert alert-warning d-flex align-items-center gap-2 mb-3 py-2 px-3 dash-banner dash-banner--warn"
        >
          <i class="ti ti-clock dash-banner-icon"></i>
          <span class="dash-banner-text"
            >You have {$missedReminderCount} unread order reminder{$missedReminderCount > 1
              ? "s"
              : ""}</span
          >
          <a
            href="/admin/notifications?read=false&type=OrderReminder"
            class="ms-auto btn btn-sm btn-outline-warning py-0">View</a
          >
        </div>
      {/if}
      ...
      {#if todayReminders.length > 0}
        <div class="card mb-3 border-0 shadow-sm dash-widget">
          <div
            class="card-header d-flex align-items-center gap-2 py-2 dash-widget-head dash-widget-head--orange"
          >
            ...
            {#each todayReminders as r}
              <div class="d-flex align-items-center gap-2 px-3 py-2 border-bottom dash-row">
                <span class="badge bg-warning text-dark flex-shrink-0 dash-badge">
                  {new Date(r.reminderTime).toLocaleTimeString("en-IN", {
                    hour: "2-digit",
                    minute: "2-digit",
                    hour12: true,
                  })}
                ...
                <a
                  href="/admin/order/{r.order?.id}"
                  class="flex-grow-1 text-dark fw-medium text-decoration-none text-truncate dash-row-title"
                >
                  {r.message || "Reminder"}
                </a>
                {#if r.order}
                  <span class="text-muted flex-shrink-0 d-none d-md-inline dash-row-meta"
                    >#{r.order.pId ?? r.order.id} – {r.order.title ?? ""}</span
                  >
                {/if}
                <button
                  class="btn btn-sm btn-light flex-shrink-0 p-0 dash-dismiss"
                  title="Dismiss reminder"
                  on:click={() => dismissTodayReminder(r.id)}>&#x2715;</button
                >
              </div>
            {/each}
          </div>
          ...
          <div class="row g-3 mb-3">
            <div class="col-6 col-md-3">
              <div class="card border-0 shadow-sm text-center py-3 dash-stat-card">
                ...
                <a
                  href={currentUser?.subRole === "tech_helper"
        ...
        <div>
          ...
          {#if currentUser?.role === "master"}
            {#if (highlights.overdueQueries?.length ?? 0) > 0}
              ...
              {@const criticalCount = highlights.overdueQueries.filter(
                (q) => q.waitingMins >= 30,
              ).length}
              <div
              ...
              >
                <div class="d-flex align-items-center gap-2">
                  ...
                  <span class="dash-alert-text">
                    {overdueCount}
                    {overdueCount === 1 ? "query" : "queries"} waiting &gt; 5 min without being picked
                    up
                    {#if criticalCount > 0}
                ...
                <div class="d-flex gap-2">
                  <a
                    ...
                    class="btn btn-sm dash-alert-btn"
                    class:dash-alert-btn--critical={criticalCount > 0}
                  >
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <div class="printDashboard">
              <div class="card flex-fill">
                <div
                  class="card-header d-flex align-items-center justify-content-between flex-wrap py-2"
                >
            ...
            {#each upcomingVisits as v}
              <a href="/admin/client-visit/{v.id}" class="uv-item text-decoration-none">
                <div class="uv-item-date">
                  <span class="uv-day"
                    >{new Date(v.visitDate).toLocaleDateString("en-GB", { day: "2-digit" })}</span
                  >
                  <span class="uv-mon"
                    >{new Date(v.visitDate).toLocaleDateString("en-GB", { month: "short" })}</span
                  >
                </div>
                <div class="uv-item-main">
                  ...
                  <div class="uv-item-meta">
                    <span class="uv-type">
                      {v.visitType === "outgoing"
                        ? "We Visit"
                        : v.visitType === "incoming"
                          ? "They Come"
                          : v.visitType === "joint"
                            ? "Joint"
                            : v.visitType}
...
<style>
  ...
  .dash-alert--critical .dash-alert-icon {
    color: #c92a2a;
  }
  .dash-alert-text {
    font-size: 12px;
  ...
  .dash-alert--critical .dash-alert-text {
    color: #c92a2a;
  }
  .dash-alert-btn {
    font-size: 11.5px !important;
  ...
  .dash-alert-btn--critical {
    background: #c92a2a;
  }

  /* Child components — keep rem inline sizes from ballooning */
  .dash-page :global(.fs-2) {
    font-size: 22px !important;
  }
  .dash-page :global(.fs-4),
  .dash-page :global(.fs-5) {
    font-size: 16px !important;
  }
  .dash-page :global(.fs-13),
  .dash-page :global(.fs-14) {
    font-size: 12px !important;
  }
  .dash-page :global(.fs-16) {
    font-size: 15px !important;
  }
  .dash-page :global(.fs-18) {
    font-size: 15px !important;
  }

  /* QueryHighlights — normalize rem-based inline text */
  .dash-page :global(.btn-sm) {
    font-size: 11.5px !important;
  }
</style>

<script>
  ...
  async function fetchOrders() {
    ...
    try {
      ...
      saveToLocalStorage("orders_lost_" + query.toString(), {
  ...
  $: columns = [
    ...
    },
    {
      ...
      label: "Last Message",
      render: (val, row) => {
        return row?.orderChats[0]?.message ? row?.orderChats[0]?.message : "-";
      },
    },
    {
      key: "lastUpdate",
      label: "Last Update",
      render: (val, row) => {
        const d = new Date(row?.orderChats[0]?.createdAt);
        return `${
          row?.orderChats[0]?.createdAt
            ? new Date(row?.orderChats[0]?.createdAt).toLocaleDateString(
                "en-GB",
                {
                  day: "2-digit",
                  month: "short",

<script>
  ...
  async function getDispatchedDetails(workOrderNumber) {
    const cached = get(dispatchedDetailsStore);
    if (cached) {
      dispatchedDetails = cached;
      loadingData = false;
      return;
    }
    loadingData = true;
    try {
      ...
      const path = `/dispatch/sales/` + workOrderNumber;
  ...
  async function cerateDispatch(e) {
    e.preventDefault();
    errorMessage = "";
    loading = true;
    formErrors = {};
                ...
                <ul class="nav nav-tabs nav-bordered border-0 mb-0">
                  ...
                  {#if order.status === "Deal Won"}
                    <li class="nav-item" role="presentation">
                      ...
                      >
                        <span class="d-md-inline-block">
                          <i class="ti ti-components me-1"></i>Multiple Orders
                        </span>
                      </a>
                    </li>
                  {/if}
                  {#if order.status === "Dispatched"}
                    <li class="nav-item" role="presentation">
                      ...
                      >
                        <span class="d-md-inline-block">
                          <i class="ti ti-components me-1"></i>Dispatched
                        </span>
                      </a>
                    </li>
                  {/if}
            ...
            <div class="tab-content pt-0">
              ...
              {#if order.status === "Dispatched"}
                {#if activeTab === "Installation"}
                  <!-- Installation -->
                  <div class="tab-pane active show" id="tab_9">
                    <div class="card">
                      <div
                        class="card-header d-flex align-items-center justify-content-between flex-wrap row-gap-3"
                      >
                        <h5 class="fw-semibold mb-0">Installation</h5>
                        <div class="d-inline-flex align-items-center">
                          <button
                            on:click={() => cerateChildOrder()}
                            class="link-primary fw-medium flex items-center"

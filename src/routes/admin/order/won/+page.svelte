<script>
  ...
  onMount(() => {
    ...
    document.addEventListener("click", (e) => {
      const target = e.target.closest(".updateImportStatus");

      if (target) {
        const id = target.dataset.id;
        changeImportStatus(id);
      }
    });
  });
  ...
  async function fetchOrders() {
    ...
    try {
      ...
      saveToLocalStorage("orders_won_" + query.toString(), {
  ...
  async function statusUpdate(order) {
    ...
    try {
      ...
      const cachedData = getFromLocalStorage("orders_won_" + query.toString());
      if (cachedData) {
        orders = cachedData.orders;
        if (order.status !== "Deal Won") {
          orders = orders.filter((o) => o.id !== order.id);
        }
        saveToLocalStorage("orders_won_" + query.toString(), { orders });
      }
      Swal.fire({
        icon: "success",
  ...
  async function importStatusUpdate(order) {
    ...
    // let newActivity = {
    //   title: "Order Import Status Changed",
    //   description: `The order import status has been updated to '${
    //     order?.importStatus
    //   } (${order.importStatus})'.`,
    // };
    // updateOrder.orderActivity = newActivity;
    try {
      ...
      if (cachedData) {
        ...
        if (index !== -1) {
          orders[index] = { ...orders[index], ...order }; // merge update
        }
      ...
      Swal.fire({
        ...
        text: "The order import status has been updated successfully.",
      });
      return true;
    } catch (error) {
  ...
  $: columns = [
    ...
    },
    {
      ...
      render: (val, row) => {
        if (currentUser?.role === "user") {
          return `<div class="flex items-center gap-1 text-success capitalize">${row.importStatus}</div>`;
        } else {
          if (row.importStatus.toLowerCase() === "true") {
            return `<div class="flex items-center gap-1 text-success capitalize cursor-pointer updateImportStatus" data-id="${row.id}">${row.importStatus}</div>`;
          }
          if (row.importStatus.toLowerCase() === "false") {
            return `<div class="flex items-center gap-1 text-danger capitalize cursor-pointer updateImportStatus" data-id="${row.id}">${row.importStatus}</div>`;
          }
        }
      },
    },
